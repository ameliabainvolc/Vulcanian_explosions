...
%%%% v1 utilising Toramaru's empirical relation with silica content to
%%%% obtain T
clear all, close all
...
sigma(i)=0.2366*exp(-0.35*(10^-6)*Pw(i) - 11*(10^3)*((1/T(i))-(1/1273))/R); % interfacial tension [N/m]
...
m(i)=(-20.79) - (5030/T(i)) - (1.4*(P(i)*10^(-6))/T(i));
m1(i)=(-20.79) - (5030/T(i)) - (1.4*(P1*10^(-6))/T(i)); %m for 1 wt% H2O
DZB2000_Cw_T_P(i)= X(i) * exp(m(i)) * (1+ exp( 56+m(i)+X(i)*(-34.1+(44620/T(i))+((57.3*P(i)*10^(-6))/T(i))) - sqrt(X(i))*(0.091 + (4.77*10^6)/T(i)^2)) ); % Diffusivity as a function of pressure, temperature and water content (Zhang and Behrens, 2000)
DBZX2004_T_P_Cs(i)= 10^( (-0.757-0.0868*Cs(i)) + ((-14785+131.7*Cs(i))/T(i)) + ((3.079-0.049*Cs(i))*(P1*10^(-6))/T(i)) ); % Diffusivity as a function of temperature, saturation pressure and silica content at 1 wt% H2O (Behrens et al. 2004)
DZB2000_1wtpc_T_P(i)= X1*exp(m1(i)) * (1+ exp (56+m1(i)+X1*(-34.1+(44620/T(i))+((57.3*P(i)*10^(-6))/T(i))) - sqrt(X1)*(0.091 + (4.77*10^6)/T(i)^2) ) ); % Diffusivity as a function of pressure, temperature and water content for a water content of 1wt% H2O (Zhang and Behrens, 2000)
D(i)= DBZX2004_T_P_Cs(i) * (DZB2000_Cw_T_P(i)/DZB2000_1wtpc_T_P(i)) ; %diffusivity of water in the melt as a function of temperature, pressure, silica content and water content (m2/s)

%Equation for decompression rate %Note abs_DR_2 has values of sigma halved
%to account for effect of CO2
abs_DR_1(i)=a*D(i)*(sigma(i)^2)*(Pw(i)^(-1/3))*(T(i)^(-1/2))*(BND(i)^(2/3)); %absolute value of the decompression rate (Pa/s)
abs_DR_2(i)=a*D(i)*((0.5*sigma(i))^2)*(Pw(i)^(-1/3))*(T(i)^(-1/2))*(BND(i)^(2/3)); %absolute value of the decompression rate (Pa/s)
end